...
const lastCar = (carInventory) => {
      return `The last car is a ${carInventory[carInventory.length - 1].car_make} ${carInventory[carInventory.length - 1].car_model}`
  }
...
// ==== Challenge 5 ==== //look at challenge 1 - add an if statement - year is less than
...
// ==== Challenge 6 ==== use an if cartype = bmw and an else if cartype = audi or use for loops
// A buyer is interested in seeing only BMW and Audi cars within the inventory. Return an array that only contains BMW and Audi cars.  Once you have populated the BMWAndAudi array, use JSON.stringify() to show the results of the array in the console.
//  use JSON.stringify() to show results

const sortModels = (carInventory) => {
  let BMWAndAudi = [];
  for (let i = 0; i < carInventory.length; i++) {
    BMWAndAudi.push(carInventory[i].car_model);
    }
    BMWAndAudi.sort();
    return BMWAndAudi.toString();
}
